...
import { FiCheck, FiSave, FiUser, FiX } from 'react-icons/fi';
import useWorkingGroup from '../working_group/hook/useWorkingGroup';
import type { Member, MemberRoll, WorkingGroup } from './members.tsx';
...
export default function EditMemberModal({ member, setMember, rolls, working_group }: EditMemberModalProps) {
    const { updateUserRoll } = useWorkingGroup();
    ...
    const handleSave = () => {
        if (!member || !selectedRoll || !working_group) return;
        const data = {
            roll_id: selectedRoll,
            user_id: member.userid,
            working_group_id: working_group.id,
        };
        updateUserRoll(data).then(res => {
            if (res.error) {
