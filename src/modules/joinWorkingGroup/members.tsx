...
import InviteUserModal from '../working_group/components/InviteUserModal';
...
export default function Menbers() {
    ...
    const { getWorkingGroupMembers, getUserRolls, inviteUserToWorkingGroup } = useWorkingGroups();
    ...
    useEffect(() => {
        if (!working_group_id) return;
        getWorkingGroupBasicData(working_group_id)
            .then(res => {
    ...
    useEffect(() => {
        ...
        getWorkingGroupMembers(workingGroup.id)
            .then(res => {
                if (res.error) {
                    console.log(res.error);
                    return;
    ...
    const handleInviteUser = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await inviteUserToWorkingGroup(workingGroup.id, inviteEmail.trim(), user.id);
