...
import {
  ...
  getRawListComponent,
  getRawCheckBoxComponent,
  getRawHelpBlockComponent,
  getRawMultipleChoisesComponent,
  getRawTextInputComponent,
  getRawGuidedCheckListComponent
} from "../components/rawComponents/getRawComponents"
...
export default function PageMenu() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        ...
        const imageItem: PageItem = getRawImageComponent(src);
        const textItem: PageItem = getRawTextComponent();
  ...
  const addList = async (listType: "ordered" | "unordered") => {
    ...
    const newList: PageItem = getRawListComponent(listType);
  ...
  const addCheckBox = async () => {
    const pageContentCopy = await JSON.parse(JSON.stringify(pageContent));
    const newList: PageItem = getRawCheckBoxComponent();
    pageContentCopy.push(newList);
    setPageContent(pageContentCopy);
  }
  ...
  const addHelpBlock = async () => {
    const pageContentCopy = await JSON.parse(JSON.stringify(pageContent));
    const newList: PageItem = getRawHelpBlockComponent();
    const textItem: PageItem = getRawTextComponent();
    pageContentCopy.push(newList);
    pageContentCopy.push(textItem);
    setPageContent(pageContentCopy);
  ...
  const addMultipleChoises = async () => {
    const pageContentCopy = await JSON.parse(JSON.stringify(pageContent));
    const newList: PageItem = getRawMultipleChoisesComponent();
    const textItem: PageItem = getRawTextComponent();
    pageContentCopy.push(newList);
    pageContentCopy.push(textItem);
    setPageContent(pageContentCopy);
  ...
  const addInputText = async () => {
    const pageContentCopy = await JSON.parse(JSON.stringify(pageContent));
    const textInputItem: PageItem = getRawTextInputComponent();
    const textItem: PageItem = getRawTextComponent();
  ...
  const addGuidedCheckList = async () => {
    const pageContentCopy = await JSON.parse(JSON.stringify(pageContent));
    const newList: PageItem = getRawGuidedCheckListComponent();
    const textItem: PageItem = getRawTextComponent();
    pageContentCopy.push(newList);
    pageContentCopy.push(textItem);
    setPageContent(pageContentCopy);
  ...
  return <div className={styles.buttonBar} style={{ zIndex: 999 }}>
