...
class Item extends React.Component<ItemProps> {
    ...
    render() {
        ...
        const { onTextChange, onGuidandeChange, activeItemId, onCollapseChange, onDeleteItem, onAddItem, onSave } =
            commonProps;
                            ...
                            label: (
                                ...
                                >
                                    ...
                                    <Input
                                        ...
                                        placeholder={
                                            item.index === 0 ? "What's the first step" : "What's the next step"
                                        }
                                        value={item.text}
...
export default function GuidedCheckList({ item }: { item: PageItem }) {
    ...
    const handleTextChange = (id: string, newText: string) => {
        setList(prevList => prevList.map(item => (item.id === id ? { ...item, text: newText } : item)));
    };

    const handleGuidandeChange = (id: string, newGuidande: string) => {
        setList(prevList => prevList.map(item => (item.id === id ? { ...item, guidande: newGuidande } : item)));
    };
