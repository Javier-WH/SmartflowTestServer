...
class Item extends React.Component<ItemProps> {
  render() {
    ...
    const { onTextChange, onGuidandeChange } = commonProps
        ...
        <Collapse
          ...
          items={[{
            ...
            children: <div className={styles.textItemContainer} style={{ marginLeft: "20px", display: "flex", flexDirection: "column", alignItems: "start", gap: "10px"}}>
                ...
                />
              <Button className={styles.nextButton} color="cyan" variant="solid">
                next
              </Button>
              </div>
          }]}
...
export default function GuidedCheckList({ item }: { item: PageItem }) {
  ...
  useEffect(() => {
    ...
    const newListItem2: ListItem = {
      id: uuidv4(),
      index: 1,
      text: "",
      guidande: ""
    }
    setList([newListItem, newListItem2])
  ...
  const _onListChange = (newList: ListItem[]) => {
    const updatedList = newList.map((item, index) => ({
      ...item,
      index: index, // Actualizar el índice según posición actual
    }));
    setList(updatedList);
  };
