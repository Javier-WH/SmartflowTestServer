...
import useRoll, { type MemberRolltype } from '@/modules/userRoll/useRoll';
...
export const MainContextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const { memberRoll } = useRoll({ userId: user?.id ?? '', working_group_id: workingGroupId ?? '' });
    const [selectedFileId, setSelectedFileId] = useState<string | null>(null);
    const [changleFileNameRequest, setChangleFileNameRequest] = useState<{ fileId: string; fileName: string } | null>(
        null,
    );
    ...
    useEffect(() => {
        if (!working_group_id && !slug) return;
        getWorkingGroupBasicData(working_group_id ?? slug ?? '')
            .then(res => {
                setWorkingGroupId(res?.data[0]?.id ?? '');
            })
            .catch(err => {
                console.log(err);
                setWorkingGroupId('');
