...
create or replace trigger before_insert_working_group_trigger before insert on public.working_group for each row when (((new.slug is null) or (new.slug = ''))) execute function public.before_insert_working_group();
...
create or replace trigger enviar_email_trigger after insert on public.working_group_invitations for each row execute function public.enviar_email();
